(function ($) {
  ...
  if (location.pathname.includes('/fr/')) {
    ...
    if (document.querySelector(`.navbar-fixed li > a[href="/fr/${href}"]`))
      document.querySelector(`.navbar-fixed li:not([class*="lang"]) > a[href="/fr/${href}"]`).classList.add('active');
  } else {
    if (document.querySelector(`.navbar-fixed li.lang-en > a`))
      document.querySelector(`.navbar-fixed li.lang-en > a`).classList.add('active');

    if (document.querySelector(`.navbar-fixed li > a[href="/${href}"]`))
      document.querySelector(`.navbar-fixed li:not([class*="lang"]) > a[href="/${href}"]`).classList.add('active');
  }
